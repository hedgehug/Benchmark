...
count_mat <- read.table('../data/count_matrix/APOBEC3C.txt_data_matrix.txt',
...
gsa=egsea.cnt(counts=cnt_mat,group=group_factor,
              gs.annots=gs.annot,symbolsMap=NULL, baseGSEAs=egsea.base()[10],
              sort.by="avg.rank", num.threads=4,report=FALSE)
# write results
write.table(gsa$results$custom$test.results$'2vs1', '../example_results/globaltest_egsea_results/APOBEC3C.egsea.globaltest.txt', quote = FALSE, sep = "\t")
