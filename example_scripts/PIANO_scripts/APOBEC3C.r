...
pval_table <- read.table('data/prerank_DESeq2/APOBEC3C.rnk')
...
tval_table <- read.table('data/prerank_fc/APOBEC3C.txt')
tvals <- setNames(pval_table$V2,pval_table$V1)


pathways <- loadGSC('pathway_files/K562_DESEq2.gmt')
...
write.table(combres$rankMat, file='example_results/PIANO_results/APOBEC3C.rank.txt', sep='\t', quote = FALSE)
